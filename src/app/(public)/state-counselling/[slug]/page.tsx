...
export default function StateDetailPage() {
  ...
  useEffect(() => {
    const st = store.states.find((s) => s.slug === slug);
    if (st) {
      setStateData(st);
      setUpdates(store.updates.filter((u) => u.state_slug === slug && u.status === "published"));
      setDates(store.dates.filter((d) => d.state_slug === slug));
      setColleges(store.colleges.filter((c) => c.state_slug === slug));
    }
  }, [slug]);
  ...
  if (!stateData) {
    return (
      <div className="py-20 max-w-4xl mx-auto px-4 text-center">
        ...
        <p className="text-sm text-slate-500 mt-2">No counselling details found for this state.</p>
        <Link
