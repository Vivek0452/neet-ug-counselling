...
export default function StateDetailPage({ params }: Props) {
  try {
    ...
    if (stateData) {
      try {
        ...
        initialUpdates = (store?.updates || []).filter(
          (u) => u && matchState(u.state_slug) && u.status === "published"
        );
        initialDates = (store?.dates || []).filter(
          (d) => d && matchState(d.state_slug)
        );
        initialColleges = (store?.colleges || []).filter(
          (c) => c && matchState(c.state_slug)
        );
      } catch (e) {
    ...
    console.error("StateDetailPage rendering fallback caught:", err);
    return (
      <StateDetailClient
        slug={params?.slug || ""}
        initialState={null}
